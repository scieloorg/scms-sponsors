...
class CsvChecker():
    ...
    def chk_bool(self, data):
        ...
        return None if data in (u'True', u'true', u'TRUE', u'T', u't', u'Yes', u'yes', u'YES', u'Y', u'y',
                                ...
                                u'No', u'no', u'NO', u'N', u'n', True,
                                False) else u"Padrão incompatível, tente ['yes', 'no', 'true', 'false', 'y', 'n']"
...
def show_errors(errlist):
    ...
    for err in errlist:
        sys.stderr.write("%s.\n" % " ".join(["%s %s" % em for em in [e for e in
                                                                     zip(("Error:", "in file", "on line", "in column"),
                                                                         err) if e[1]]]))
...
def check_csv_file(csv_fname, cols, halt_on_err, columnexit, \
    ...
    if len(req_missing) > 0:
        errorlist.append(("The following columns are required, but are not present in the CSV file: %s." % ", ".join(
            req_missing), csv_fname, 1))
        return errorlist
    ...
    if columnexit:
        ...
        if len(extra) > 0:
            errorlist.append((
                "The following columns have no format specifications but are in the CSV file: %s." % u", ".join(
                    extra), csv_fname, 1))
            return errorlist
...
if __name__ == '__main__':
    try:
        ...
        lno = strace[0][1]
        src = strace[0][3]
        sys.stderr.write("%s: Uncaught exception %s (%s) on line %s (%s)." % (
            os.path.basename(sys.argv[0]), str(sys.exc_info()[0]), sys.exc_info()[1], lno, src))
        sys.exit(1)
    sys.exit(status)
