...
def validate(request):
    ...
    if file_id:
        file_upload = get_object_or_404(SponsorNamesStandardizedFile, pk=file_id)
...
def import_file(request):
    ...
    try:
        with open(file_path, 'r') as csvfile:
            ...
            for row in data:
                isd = SponsorNamesStandardized()
                isd.sponsor_id = row['ID']
                isd.sponsor_name_std = row['Standard_Name']
                isd.sponsor_acron_std = row['Standard_Acronym']
